...
st.subheader("Interface mobile - Police Municipale")
...
st.write("### 👤 Informations Agent, Période & Hiérarchie")
col_a, col_b = st.columns(2)

with col_a:
    agent_nom = st.text_input("Nom et Prénom de l'agent :", "BOTTE Philippe")
    service_nom = st.text_input("Service / Direction :", "Police Municipale")
    mois_annee = st.text_input("Mois / Année (ex: JUILLET 2026) :", "JUILLET 2026")

with col_b:
    resp_hierarchique = st.text_input("Responsable hiérarchique (Bas de page) :", "NOM Prénom")
    resp_direct = st.text_input("Responsable hiérarchique direct (Bas de page) :", "Teddy TOURVILLE")
...
if st.button("➕ Enregistrer cette vacation", use_container_width=True):
    # Calcul d'amplitude avec passage de minuit
...
if st.session_state.registre_heures:
    ...
    if st.button("📥 Générer le fichier Excel officiel", type="primary", use_container_width=True):
        try:
            ...
            ws["C6"] = agent_nom
            ws["F6"] = f"Mois :  {mois_annee}"
            ws["C8"] = service_nom

            # Remplissage dynamique des visas en bas de page
            ws["C36"] = resp_hierarchique
            ws["C37"] = resp_direct

            # Nettoyage sécurisé des anciennes lignes de données (Lignes 13 à 27)
            ...
            # Injection des nouvelles données à partir de la ligne 13
            start_row = 13
            for idx, data in enumerate(st.session_state.registre_heures):
                r_idx = start_row + idx
                if r_idx > 27:
                    st.warning("⚠️ Capacité maximale du tableau atteinte pour ce mois.")
                    break
            ...
            with open(output_filename, "rb") as f:
                st.download_button(
                    label="💾 Télécharger l'Excel pour la hiérarchie",
                    data=f,
